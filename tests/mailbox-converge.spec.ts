...
test("a watermark is bound to its replica: a count from a shed identity reads as zero", () => {
   ...
   *
   * A seq numbers rows within the replica that issued them and nowhere else. A
   * copy that arrives by file adopts a *new* identity when it opens (T1-D22),
   * and it has published none of its rows under that identity — but a watermark
   * carried over from the identity it shed names a seq in the sender's sequence
   * space. Read as a bare number that seq sits above this copy's own first rows
   * and strands them, so the mailbox asks for what was authored above a
   * `(replica, seq)` pair, not a number: a watermark whose replica is not the
   * one this copy now writes as is read as zero, and the first row under the new
   * identity is sent, reported under the identity it was actually authored as.
   */
  const a = open();
  ensureReplica(a, A);
  ...
  // This copy takes a new identity, as a file arrived from A does on open. The
