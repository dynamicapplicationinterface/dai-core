...
| Seat | A seat the session's creator minted, held by the first verified signer of a binding to it. The seat tables are owned by the kit: `daiKit.newSession`, `claimSeat` and `reseat` are their only writers, and `amCreator`, `mySeat` and `seats` are kit reads, built on the host's author id and never on a row. An app that writes them directly fails a check (`seat-table-write` in the authoring lint; `scripts/check-seats.mjs` for this repository). A table whose rows act for a seat names the seat in a column (`-- dai:replicated seat=<column>`). | app kit, `daiKit` |
...
5. **A seat is a signed claim.** `amCreator` and its kin ask "does the creator seat's batch verify under my key", never "does the author column equal my id". A copy holding a different key cannot forge a seat. **A signature answers who wrote a row; a seat answers whether they may.** The signature alone does not close D80: a copy that honestly signs as Bo can still play White's move, and only the seat can refuse it. A row in a seated table names the seat it acts for, and is admitted only when its author held that seat when the row was written: the author's binding holds the seat and is at or before the row's clock, and the seat had that value then, with no reseat between. Held then, not held now: a move written before its author took the seat never becomes a move. A row naming no seat, or a seat outside its session, is stored and never admitted, and a merge reports what it took and did not admit as `SEAT_NOT_HELD` with its author (step 5; `IDENTITY-SEAT-ADMITS` in `src/rules.ts`). The check is the document's compiled admission, not the application's.
