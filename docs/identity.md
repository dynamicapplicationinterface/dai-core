...
A person is a keypair made once on their device and held there. Everything they write into a document is signed by that key. Whether that key belongs to a real, named person is a separate layer: empty unless an authority, such as a firm's identity provider, attests the key.
...
**Verification** is optional and starts empty. It answers "whose key is this?" with a signed statement from an authority: "key K belongs to Jane in Legal." Where no authority vouches, the layer stays empty. The authority can be any issuer a verifier trusts, such as a firm's existing identity provider. This sitting reserves the space for it and builds none of it.
...
| Seat | A seat the session's creator minted. The session id commits to the creator (SHA-256 of the creator's author id and a nonce, first 16 bytes, the nonce on the creator's own seat row); the creator's seat is the creator's by definition; the open seat is held by whoever the creator's copy confirms. No clock decides a seat. The seat tables are owned by the kit: `daiKit.newSession`, `claimSeat` and `reseat` are their only writers (the kit also confirms, on the creator's copy, by itself), and `amCreator`, `mySeat`, `pendingSeat` and `seats` are kit reads, built on the host's author id and never on a row. An app that writes them directly fails a check (`seat-table-write` in the authoring lint; `scripts/check-seats.mjs` for this repository). A table whose rows act for a seat names the seat in a column (`-- dai:replicated seat=<column>`). | app kit, `daiKit` |
...
| Publisher key | The key that signs a container for the store and the version relay. One key per person, two roles: for a solo author the publisher role is filled by their person key. `publish()` takes a key rather than assuming the person's, so a key other than the person's (a firm's, say) can publish a container whose rows other people sign. | existing, `src/publish.ts` |
...
3. **Every write is a signed batch.** The frame assembles the batch, sends it to the host over the bridge, the host signs, the batch is stored and replicated with its signature. No unsigned batch is written by a runtime at this version or later. *The seal is a courtesy to the honest path; the verifier is the enforcement.* The frame belongs to the document, so nothing it does can be relied on: the host checks the bytes that leave (a row of this author's that is pending, or names a header the bytes do not hold, refuses the leave), and what stops a hostile copy is the merge that verifies (rule 4).
4. **The merge verifies before it applies.** A batch whose signature does not verify under the author id it names is refused, not applied, and reported (this is D82's other half: the merge can now say whose row it refused). A batch from an author id never seen before is accepted as a new author. Restricting new authors would be a per-document policy the merge checks; none exists at this version.
5. **A seat is a signed claim.** `amCreator` and its kin ask "does the creator seat's batch verify under my key", never "does the author column equal my id". A copy holding a different key cannot forge a seat. **A signature answers who wrote a row; a seat answers whether they may.** The signature alone does not close D80: a copy that honestly signs as Bo can still play White's move, and only the seat can refuse it. A row in a seated table names the seat it acts for, and is admitted only when its author holds that seat. The session id commits to its creator, so who created it is checked from the rows; the creator's seat is the creator's; the open seat is held by whoever the creator's copy confirms, in a row only the creator writes, which the kit writes when it sees exactly one copy asking and withholds when it sees two (a contest, for the creator to repair). No clock decides anything, and a hold never moves once made, so a move written while its author waited to be seated is admitted once they are, and one written for a seat they never held never is. A seat is the pair (session, seat): anyone can create a session and give its seat any bytes, so a side is read from a row's session and seat together, and a game's rows only in its own session. An entity belongs to the session it was written in, and a row naming an earlier version from another session is never admitted (`ENTITY_OTHER_SESSION`); in a seated table a row replaces only rows of its own seat, and one naming another seat's row is `SEAT_NOT_HELD`. The writers read an entity the way admission does: a change, a delete or a reseat versions only the heads the admitted view shows in the one session and seat this copy writes in, never every row that shares the id (D135 to D138). A row naming no seat, or a seat someone else holds, is stored and never admitted, and a merge reports it as `SEAT_NOT_HELD` with its author; a row waiting on a confirmation is neither admitted nor reported (step 5; `IDENTITY-SEAT-ADMITS` and `IDENTITY-SEAT-CONFIRMED` in `src/rules.ts`). The check is the document's compiled admission, not the application's.
...
The fields are reserved so that adding an attestation changes no signed byte: a document signed at this version reads and verifies the same whether or not its batches ever carry one.

| Field | Where | V1 | When used |
| --- | --- | --- | --- |
...
| Seat secret in the invite fragment (D80 option d) | fragment key | **not reserved: superseded** | signed seats make a per-seat secret unnecessary; the joiner seat is held by whoever the creator's copy confirms (ruled 24 Sept, replacing "the first verified signer") |
...
**Consumer.** Grace opens a `.dai`. Her phone makes a key; she sees nothing. She writes; every batch is signed. She shares a copy with Dan; Dan's phone makes its own key and Dan's moves are Dan's. Nobody logged in, nothing was uploaded, no server knows either of them. If an authority ever vouches for her key, every app she made becomes attributable to her, because they were signed all along.

**Enterprise.** Jane opens the same format at work. Her key is made the same way. The firm's identity provider signs an attestation binding Jane's key to her directory identity, and that attestation rides in `att` on her batches. An auditor verifies the chain: batch → key → attestation → the firm's issuer. Same documents, same signatures, now with a name bound to the key by an issuer the auditor trusts. The format did not change; a link was added at the top.

**A policy is the only runtime addition attestation needs.** A document could accept batches only from keys attested under a given issuer. That would be a rule the merge checks, and the one place rule 4's "accept any new author" is overridden. Everything else is the same runtime.

An unattested document is an attested one with the top link absent: the chain ends at the key. So one format and one verifier serve both.

## Key durability, loss, second device

The key is stored in the host's IndexedDB under a key name owned by `src/keys.ts`, as a WebCrypto key marked extractable. Extractable is required: a key that cannot be exported cannot become a recovery phrase, so it cannot be recovered on another device or vouched for there later. Persistence is asked under D55's rule, after the first real write, never at boot.

**Loss.** If storage is wiped, the device mints a new key and becomes a new author. Old documents stay readable and mergeable; the person can no longer write as their old author id. This is the accepted V1 behavior, stated on screen through a one-line kit hook whose default is "This device is a new player here. Your earlier moves are still on the board." Apps may say it their own way with `daiKit.onNewPlayer(fn)`; none says identity, key or storage. The host decides when it applies: it made the key on this page, and its library says this device wrote the document before (`IDENTITY-LOSS-SENTENCE` in `src/rules.ts`). Nothing pretends continuity that does not exist. The sentence covers a partial loss, where the library remembers the document and the key is gone. A full wipe cannot be detected from what is left on the device: with the library gone too, the device has nothing that says it ever wrote the document, and it opens as a new player without being told. A recovery phrase is what would change that, by letting the old key come back; it is not built.

**Second device.** Two ways one author can write from two devices; neither is built:
...
The example apps (chess, tic-tac-toe, receipts, request, Moon Garden) are rebuilt on the new runtime rather than migrated. Their pre-identity documents on any test phone are discarded. An app first built at this version starts signed, so it never carries a legacy batch.
...
1. **Recipient is not the creator.** iPhone UA, store link with data, fake store, through the relaunch: recipient's author id ≠ creator's, waiting to be seated, name prompt once seated. Red on `7653c44`. Desktop stays green before and after.
...
6. **A new device is a new author.** Fresh context, same document arriving: different author id, waiting to be seated; the name prompt once the creator's copy seats it (ruled 24 Sept: a name written while waiting would wait on the same confirmation).
...
- [x] The seat model: the session id commits to its creator (SHA-256 of the creator's author id and a nonce, first 16 bytes, the nonce on the creator's first seat row); the creator's seat is the creator's by definition; the open seat is held by whoever the creator confirms, in a row only the creator writes, which the kit writes automatically when it sees exactly one binding, and shows a contest when it sees two. No clock decides anything. It replaces the first-verified-signer rule, withdrawn with the attacks that broke it (`src/rules.ts`, and `tests/seat-attacks.spec.ts`).
- [x] A move waiting on a confirmation is pending: neither admitted nor refused, with no new report field. The kit's `pendingSeat(session)` says a copy is waiting; its rows wait in `t_pending`. `SEAT_NOT_HELD` is reported only for a row naming no seat or a seat someone else holds.
- [x] A parent outside the row's own entity hides nothing (backlog finding 5; `T1-D35` in `docs/replicated-tables.md`).
- [x] Loss: the partial-loss trigger stays; a full wipe cannot be detected, and a recovery phrase is what would change that (above, under Loss).
