 ...
 * or as a `case` label in a switch on one (`case "dai:x":`). Not seen: a name
 * built in a template literal, and a `{ type:` split across lines (backlog D106).
...
export function literalProblems(
  ...
  const asType =
    /type\s*(?::|===|!==|==|!=)\s*["'](dai:[a-z0-9-]+)["']|["'](dai:[a-z0-9-]+)["']\s*(?:===|!==|==|!=)\s*[\w.?]*type\b|\bcase\s+["'](dai:[a-z0-9-]+)["']\s*:/g;
  const problems: string[] = [];
  const used = new Set<string>();
  for (const { path, text } of files) {
    text.split(/\r?\n/).forEach((line, index) => {
      ...
      const found = [
        ...
        ...[...line.matchAll(asType)].map((m) => (m[1] ?? m[2] ?? m[3])!),
      ];
